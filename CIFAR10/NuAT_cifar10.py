...
#Defaults
EXP_NAME = 'CIFAR10_ResNet18_NuAT'
l_ce = 1.0
Nuc_reg = Nuc_max = 4.5
TRAIN_BATCH_SIZE = 64
Feps = 8.0
...
def NucAttack(model,loss,image,target,eps=8/255,bounds=[0,1],steps=1,out=[],Nuc_reg=4,alt=1): 
    tar = Variable(target.cuda())
    img = image.cuda()
    eps = eps/steps 
    for step in range(steps):
        ...
        if alt:
            cost = loss(rout,tar) + Nuc_reg*torch.norm(out - rout, 'nuc')/TRAIN_BATCH_SIZE
        else:
            cost = loss(rout,tar)
        cost.backward()
        per = eps * torch.sign(img.grad.data)
...
scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0, max_lr=lr_max,
            step_size_up=lr_steps *lr_up, step_size_down=lr_steps *(epochs-10-lr_up))
...
for epoch in range(epochs):
    ...
    for data, target in train_loader:
        ...
        adv_data = NucAttack(model,loss,adv_data,target,eps=Feps/255.0,steps=1,out=out,Nuc_reg=Nuc_reg,alt=(counter%2))
    ...
    if (epoch - lr_up - lr_down) in [5,10]:
        LR /= lr_factor
        for param_group in optimizer.param_groups: 
            param_group['lr'] = LR
    
    if epoch <(epochs-10):
        Nuc_reg += Nuc_max/(epochs-10)
