...
#Defaults
EXP_NAME = 'CIFAR10_WideResNet34_NuAT'
l_ce = 1.0
Nuc_reg = Nuc_max= 4.0
TRAIN_BATCH_SIZE = 64
Feps = 8.0
...
def NucAttack2(model,loss,image,target,eps=8./255.,bounds=[0,1],steps=2,Nuc_reg=4):
    ...
    for step in range(steps):
        ...
        cost = loss(rout,tar)
        if (step)%2==0:
            cost += Nuc_reg*torch.norm(out - rout, 'nuc')/img.size(0)
        cost.backward()
        per = eps * torch.sign(img.grad.data)
        adv = img.data + per.cuda() 
        img = torch.clamp(adv,bounds[0],bounds[1])
...
scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.0, max_lr=lr_max,
            step_size_up=lr_steps *lr_up, step_size_down=lr_steps *(lr_down+5))
...
for epoch in range(epochs):
    ...
    for data, target in train_loader:
        ...
        model.eval()
        adv_data = NucAttack2(model,loss,data,target,eps=Feps/255.0,steps=2,Nuc_reg=Nuc_reg)       
        model.train()
        optimizer.zero_grad()
